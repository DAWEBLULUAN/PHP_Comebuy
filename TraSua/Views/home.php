<?php 
	require 'parts/header.php';
	require 'parts/fixed-sidebar.php';
 ?>

			<header class="header">

				<div class="container">

					<!-- Menu here -->
					<?php include_once 'parts/body.navbar.php'; ?>
					<!-- End menu -->
					
					<!--Caption-->
					<div class="caption">

						<!--Made with love-->

						<div class="made_with_love col-lg-12 col-md-12 col-sm-12 col-xs-12">MADE WITH LOVE</div>

						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 col-sm-12"><img src="Views/img/icons/like.png" alt="heart"></div>

						<!--End-->

						<!-- Phrase -->
						<h1 class="title col-lg-12 col-md-12 col-sm-12 col-xs-12">GOOD TEA - GOOD DAY</h1>
						
						<div class="phrase col-lg-12 col-md-12 col-sm-12 col-xs-12">
							THE DAY BEGINS WITH TEA
						</div>
						<!-- End Phrase -->

					</div>


				</div>

				

			</header>

		</div>
		<div id="my-content">
			<br><br><br>

	<!-- $.get("phantrang_sp.php",{maloai:loaisp,indexT:indexTrangg},function(data){
  	  	$("#phan_trang").html(data);
 	   }) -->


			<div class="six-kinds w3-hide-small">
				<div class="l-item w3-display-container">
					<a href="menu.php?route=homeToMenu&&maloai=LSP001">
						<img src="View/img/bg-banner/bg-banner-1.jpg" alt="" class="w3-display-position bg-img">
						<div class="w3-text-white w3-bold w3-large w3-uppercase w3-display-topleft w3-display-hover">teapresso</div>
						<img src="View/img/bg-banner/teapresso.png" alt="" class="img-of-kind w3-display-position">
						<img src="Views/img/bg-banner/bg-banner-1.jpg" alt="" class="w3-display-position bg-img">
						<div class="w3-text-white w3-bold w3-large w3-uppercase w3-display-topleft w3-display-hover">teapresso</div>
						<img src="Views/img/bg-banner/teapresso.png" alt="" class="img-of-kind w3-display-position">
					</a>
				</div>
				<div class="l-item w3-display-container">
					<a href="menu.php?route=homeToMenu&&maloai=LSP002">
						<img src="Views/img/bg-banner/bg-banner-2.jpg" alt="" class="w3-display-position bg-img">
						<div class="w3-text-white w3-bold w3-large w3-uppercase w3-display-topleft w3-display-hover">milk tea</div>
						<img src="Views/img/bg-banner/tra-sua-tran-chau.png" alt="" class="img-of-kind w3-display-position">
					</a>
				</div>
				<div class="l-item w3-display-container">
					<a href="menu.php?route=homeToMenu&&maloai=LSP003">
						<img src="Views/img/bg-banner/bg-banner-3.jpg" alt="" class="w3-display-position bg-img">
						<div class="w3-text-white w3-bold w3-large w3-uppercase w3-display-topleft w3-display-hover">cream teaccino</div>
						<img src="Views/img/bg-banner/teaccino.png" alt="" class="img-of-kind w3-display-position">
					</a>
				</div>
				<div class="l-item w3-display-container">
					<a href="menu.php?route=homeToMenu&&maloai=LSP004">
						<img src="Views/img/bg-banner/bg-banner-4.jpg" alt="" class="w3-display-position bg-img">
						<div class="w3-text-white w3-bold w3-large w3-uppercase w3-display-topright w3-display-hover">other tea</div>
						<img src="Views/img/bg-banner/latte.png" alt="" class="img-of-kind w3-display-position">
					</a>
				</div>
				<div class="l-item w3-display-container">
					<a href="menu.php?route=homeToMenu&&maloai=LSP005">
						<img src="Views/img/bg-banner/bg-banner-5.jpg" alt="" class="w3-display-position bg-img">
						<div class="w3-text-white w3-bold w3-large w3-uppercase w3-display-topright w3-display-hover">fruit tea</div>
						<img src="Views/img/bg-banner/juice.png" alt="" class="img-of-kind w3-display-position">
					</a>
				</div>
				<div class="l-item w3-display-container">
					<a href="menu.php?route=homeToMenu&&maloai=LSP006">
						<img src="Views/img/bg-banner/bg-banner-6.jpg" alt="" class="w3-display-position bg-img">
						<div class="w3-text-white w3-bold w3-large w3-uppercase w3-display-topright w3-display-hover">ice blended</div>
						<img src="Views/img/bg-banner/Ice-Blend.png" alt="" class="img-of-kind w3-display-position">
					</a>
				</div>
			</div>
			<!-- End Our Best section -->


			<!-- Comments section -->
			<div class="comments">

				<div class="container">

					<!-- Caption -->
					<div class="caption">

						<div class="caption_upper col-lg-12 w3-text-black">
							Our Best
						</div>

						<div class="caption_lower_brown col-lg-12 w3-text-black">
							~ PEOPLE LIKE IT ~
						</div>
						<!-- End Caption -->

					</div>

					<div class="single_comment col-lg-12">

						<div class="owl-carousel">

							<!-- Comment 1 -->
							<div class="item">

								<a href="menu.php"><div class="comment_img w3-text-black"><img src="Views/img/tea/1.png" alt=""></div></a>
								<!-- <div><a href="http://facebook.com">MARY JANE</a></div> -->

								<div class="best_coffee_name">
									<a href="menu.php" class="w3-text-black">Latte with vanila</a>
								</div>

								<div class="best_phrase w3-text-black">
									taste and enjoy new coffee with special additive
								</div>

							</div>
							<!-- End Comment 1 -->

							<!-- Comment 2 -->
							<div class="item">

								<a href="menu.php"><div class="comment_img"><img src="Views/img/tea/2.png" alt=""></div></a>
								<!-- <div><a href="http://facebook.com">MARY JANE</a></div> -->

								<div class="best_coffee_name">
									<a href="menu.php" style="color: #715b2a !important;">Latte with vanila</a>
								</div>

								<div class="best_phrase">
									taste and enjoy new coffee with special additive
								</div>

							</div>
							<!-- End Comment 2 -->

							<!-- Comment 3 -->
							<div class="item">

								<a href="menu.php"><div class="comment_img"><img src="Views/img/tea/3.png" alt=""></div></a>
								<!-- <div><a href="http://facebook.com">MARY JANE</a></div> -->

								<div class="best_coffee_name">
									<a href="menu.php" style="color: #715b2a !important;">Latte with vanila</a>
								</div>

								<div class="best_phrase">
									taste and enjoy new coffee with special additive
								</div>

							</div>
							<!-- End Comment 3 -->
							
							<div class="item">

								<a href="menu.php"><div class="comment_img"><img src="Views/img/tea/4.png" alt=""></div></a>
								<!-- <div><a href="http://facebook.com">MARY JANE</a></div> -->

								<div class="best_coffee_name">
									<a href="menu.php" style="color: #715b2a !important;">Latte with vanila</a>
								</div>

								<div class="best_phrase">
									taste and enjoy new coffee with special additive
								</div>

							</div>

							<div class="item">

								<a href="menu.php"><div class="comment_img"><img src="Views/img/tea/5.png" alt=""></div></a>
								<!-- <div><a href="http://facebook.com">MARY JANE</a></div> -->

								<div class="best_coffee_name">
									<a href="menu.php" style="color: #715b2a !important;">Latte with vanila</a>
								</div>

								<div class="best_phrase">
									taste and enjoy new coffee with special additive
								</div>

							</div>

						</div>

					</div>

				</div>

			</div>

		</div>

		<!-- END CONTENT -->

		<!-- FOOTER -->
		<?php include_once 'parts/footer.php'; ?>
		
		<!--END FOOTER -->